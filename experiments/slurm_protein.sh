...
#SBATCH --output=logs/random_ell1_gsm_protein_%a.log # name logs

n=$SLURM_ARRAY_TASK_ID                  # define n  
line=`sed "${n}q;d" params_protein.txt`      # get n:th line (1-indexed) of the file 

export OMP_NUM_THREADS=8

sleep $n
srun -J protein-$SLURM_ARRAY_TASK_ID python -u run-experiment.py $line
srun -J protein-$SLURM_ARRAY_TASK_ID python -u run-experiment.py $line
